classdef ZOHA_Cylind < handle
	properties
        ...
        sphere_flag = true  % initialize the whole system as linear?
        lr_sph 
        mu_sph 
    end
      ...
      % (space_dimen, population_size=40, lr_norm=0.5, mu_norm=5, lr_sph=2, mu_sph=0.005,
        ...
        function parseParameters(self, opts)
            ...
            self.lr_norm = opts.lr_norm  % learning rate (step size) of moving along gradient
            self.mu_norm = opts.mu_norm  % scale of the Gaussian distribution to estimate gradient
            self.lr_sph = opts.lr_sph
            self.mu_sph = optsmmu_sph
            ...
            ExpectExplAng = (sqrt(self.dimen) * self.mu_norm) / pi * 180; % Expected angular distance between sample and basis 
        ...
        function [new_samples] = doScoring(self, scores)
        N = self.dimen;
...
%         end
        if self.istep == -1
            ...
            if self.rankweight == false: % use the score difference as weight
                ...
                % weights = (self.scores - self.xscore) / self.scores.size % / self.mu
                ...
                if self.maximize == false:  % note for weighted recombination, the maximization flag is here.
                    [~code_rank] = ismember(rankedscore, sort(rankedscore,'ascend'));
                else:
                    [~code_rank] = ismember(rankedscore, sort(rankedscore,'descend'));
                end 
            ...
            if self.sphere_flag == false:
                new_samples(2:end, :) = self.xnew + self.tang_codes % Linear space ExpMap reduced to just normal linear addition.
            else:  % Spherically move mean
                new_samples(2:end, :) = ExpMap(self.xnew, self.tang_codes) % m + sig * Normal(0,C) self.mu *
            end
